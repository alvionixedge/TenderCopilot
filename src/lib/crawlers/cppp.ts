...
/** Marks a failure the retry loop must rethrow immediately instead of retrying. */
...
export async function fetchListingPage(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      const message = `CPPP ${listing} p${page} responded ${res.status} ${res.statusText}`;
      if (!isRetriableStatus(res.status)) throw new FatalFetchError(message);
